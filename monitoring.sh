#!/bin/bash
arc=$(uname -a)
pcpu=$(grep "cpu cores" /proc/cpuinfo | sort | uniq | wc -l)   #cpu cores is the physical core inside the cpu
vcpu=$(grep "processor" /proc/cpuinfo | wc -l)
fram=$(free -m | awk '$1 == "Mem:" {print $NF}')	# NF stores the number of fields; it prints whats inside the last field
